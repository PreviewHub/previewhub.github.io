var HTMLPreview = {
	...
	replaceAssets: function() {
		...
		for(i = 0; i < a.length; ++i) {
			...
			//Check if it's from raw.github.com or bitbucket.org and to HTML|Markdown files
			else if((href.indexOf('//raw.githubusercontent.com') > 0 || href.indexOf('//bitbucket.org') > 0) 
				&& (href.indexOf('.html') > 0 || href.indexOf('.htm') > 0 ||
				href.indexOf('.markdown') > 0 || href.indexOf('.md') > 0 || href.indexOf('.mkdn') > 0 || href.indexOf('.mdown') > 0)) { 
	...
	loadHTML: function(data) {
		...
		&& data.query.diagnostics.redirect) {
			...
			setTimeout(function() {
				...
				HTMLPreview.content.innerHTML = markdown.toHTML(content);
				HTMLPreview.replaceAssets();
				//document.close();
	...
	submitform: function() {
		location.href = '/markdown/?' + document.getElementById('file').value;
		return false;
	},
